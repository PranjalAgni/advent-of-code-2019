const getGraphAndVertex = input => {
  ...
  for (const data of input) {
    ...
    if (!vertex.includes(nodes[1])) vertex.push(nodes[1]);
  }
...
const findDirectAndIndirectOrbits = input => {
  const [graph, vertex] = getGraphAndVertex(input);
...
const minOrbitalsTransfers = input => {
  const graph = buildGraph(input);
  const [, vertex] = getGraphAndVertex(input);

  let fromNode = graph['YOU'][0];
  let toNode = graph['SAN'][0];
...
console.log(minOrbitalsTransfers(input));
